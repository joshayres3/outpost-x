...
const PORTAL_DISCORD_GUILD_ID = String(process.env.WATCHER_GUILD_ID || process.env.DISCORD_GUILD_ID || process.env.GUILD_ID || '1516269432538661025').trim();
const PORTAL_REGISTER_CHANNEL_ID = String(process.env.WATCHER_REGISTER_CHANNEL_ID || '1517255357888466964').trim();
...
function discordOAuthConfig() {
  ...
  const guildId = PORTAL_DISCORD_GUILD_ID;
...
function discordClientReady() {
  return !!(botRef?.isReady?.() || botRef?.readyAt);
}

async function waitForDiscordReady(timeoutMs = 12_000) {
  if (discordClientReady()) return true;
  if (!botRef?.once) return false;
  return new Promise((resolve) => {
    ...
    const finish = (value) => {
      ...
      settled = true;
      clearTimeout(timer);
      botRef.off?.('ready', onReady);
      resolve(value);
    };
    const onReady = () => finish(true);
    const timer = setTimeout(() => finish(discordClientReady()), timeoutMs);
    timer.unref?.();
    botRef.once('ready', onReady);
  });
}

async function withPortalTimeout(promise, timeoutMs, label) {
  let timer;
  try {
...
async function resolvePortalGuildMember(userId, preferredGuildId) {
  ...
  let guild = preferred ? botRef.guilds?.cache?.get(preferred) : null;
  if (!guild && preferred && discordClientReady()) {
    guild = await withPortalTimeout(
      botRef.guilds.fetch(preferred),
      8_000,
      'Discord server verification'
    ).catch(() => null);
  }

  // Safe fallback only when Watcher is connected to exactly one server. This
  // handles a stale/missing Railway guild variable without authorizing a user
  // from an unrelated server.
  if (!guild) {
    const cachedGuilds = [...(botRef.guilds?.cache?.values?.() || [])];
    if (cachedGuilds.length === 1) {
      guild = cachedGuilds[0];
      if (preferred && String(guild.id) !== preferred) {
        console.warn(`⚠️ Portal guild ${preferred} was unavailable; using Watcher's only connected guild ${guild.id}.`);
      }
    }
  }
  ...
  if (!member) {
    member = await withPortalTimeout(
...
async function refreshPortalSessionAccess(session, force = false) {
  ...
  const resolved = await resolvePortalGuildMember(session.discordId, session.guildId);
  if (!resolved || !hasPortalAccess(resolved.member)) return null;
  session.guildId = String(resolved.guild.id);
  session.displayName = resolved.member.displayName || session.displayName || 'Outpost Player';
  session.isAdmin = hasTrackerRole(resolved.member);
  session.isOwner = hasOwnerRole(resolved.member);
...
async function requireRegisteredPortalPlayer(session) {
  ...
  if (!/^\d{15,20}$/.test(steamId)) {
    throw new Error('Register your SCUM character before using Command Center player services.');
  }
  return link;
}
...
async function buildPortalOverview(session) {
  const link = await portalLink(session);
  const steamId = String(link?.steam_id || '').trim();
  const registered = /^\d{15,20}$/.test(steamId);
  const permissions = session.isAdmin ? await getAdminPermissions(getDb(), session.guildId) : {};

  // Regular Discord members can enter the portal and register there, but no
  // protected player data is returned until a SCUM link exists.
  if (!registered && !session.isAdmin && !session.isOwner) {
    const portalSettings = await loadPortalSettings(session.guildId);
    return {
      me:{discordId:session.discordId,displayName:session.displayName||link?.discord_tag||'Outpost Player',avatar:session.avatar||null,isAdmin:false,isOwner:false,registered:false,permissions:{},permissionCatalog:[],sessionExpiresAt:new Date(session.expiresAt).toISOString()},
      registrationUrl:PORTAL_REGISTER_URL,
      player:{steamId:null,name:null,online:false,cash:null,fame:null},
      vehicles:[],insurance:[],rental:null,airlift:{ready:false,nextRide:null},
      shops:[],myShop:null,squads:[],mySquad:null,lore:[],myLore:null,events:[],transactions:[],
      shopCatalog:[],
  ...
  const portalSettings = await loadPortalSettings(session.guildId);
  return {
    me:{discordId:session.discordId,displayName:session.displayName||link?.discord_tag||'Outpost Player',avatar:session.avatar||null,isAdmin:!!session.isAdmin,isOwner:!!session.isOwner,registered,permissions,permissionCatalog:session.isOwner?permissionCatalog():[],sessionExpiresAt:new Date(session.expiresAt).toISOString()},
    registrationUrl:PORTAL_REGISTER_URL,
...
async function handleHttp(req, res) {
  ...
  if (url.pathname === '/portal/oauth/callback') {
    ...
    try {
      ...
      if (!resolved) {
        throw new Error(discordClientReady()
  ...
  // Old saved registration links and the portal registration button both land
  // on the protected dashboard, where the player can generate a code.
  ...
  if (url.pathname === '/portal/api/registration' && req.method === 'POST') {
    try { return json(res, 200, await portalStartRegistration(portalCtx(session))); }
