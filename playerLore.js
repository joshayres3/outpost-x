...
async function portalCreateLore(ctx,values){const existing=await getLoreByOwner(ctx.guildId,ctx.discordId);if(existing)throw new Error('You already have a lore profile.');const cfg=await getConfig(ctx.guildId);if(!cfg?.channel_id)throw new Error('Player Lore has not been set up in Discord yet.');const guild=clientRef?.guilds?.cache?.get(String(ctx.guildId));const channel=guild?.channels?.cache?.get(String(cfg.channel_id));if(!channel?.isTextBased())throw new Error('The Player Lore channel is unavailable.');const name=String(values.character_name||'').trim().slice(0,80);const backstory=String(values.backstory||'').trim().slice(0,3500);if(!name||!backstory)throw new Error('Character name and backstory are required.');const now=new Date().toISOString();const temp=await channel.send({embeds:[new EmbedBuilder().setDescription('Creating lore profile…')]});const cat=LORE_CATEGORIES.find(c=>c.key===String(values.category)||c.label===String(values.category))||LORE_CATEGORIES[0];const row={guild_id:String(ctx.guildId),owner_id:String(ctx.discordId),owner_display:String(ctx.displayName||'Player').slice(0,100),character_name:name,title_nickname:String(values.title_nickname||'').trim().slice(0,100),faction_squad:String(values.faction_squad||'').trim().slice(0,100),origin:String(values.origin||'').trim().slice(0,200),backstory,personality:String(values.personality||'').trim().slice(0,1000),goals:String(values.goals||'').trim().slice(0,1000),extra_lore:String(values.extra_lore||'').trim().slice(0,1000),rp_status:['Open to RP','Casual RP','Not Seeking RP'].includes(values.rp_status)?values.rp_status:'Casual RP',category:cat.label,image_urls:[],image_caption:String(values.image_caption||'').trim().slice(0,1500),is_published:true,channel_id:String(cfg.channel_id),message_id:String(temp.id),created_at:now,updated_at:now};const{data,error}=await dbRef.from(LORE_TABLE).insert(row).select('*').single();if(error){await temp.delete().catch(()=>{});throw error;}await temp.edit({embeds:loreEmbeds(data),allowedMentions:{users:[]}});await updateDirectory(ctx.guildId);return data;}
async function portalUpdateLore(ctx,values){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const cat=LORE_CATEGORIES.find(c=>c.label===values.category||c.key===values.category)||categoryForEntry(entry);const patch={character_name:String(values.character_name??entry.character_name).trim().slice(0,80),title_nickname:String(values.title_nickname??entry.title_nickname??'').trim().slice(0,100),faction_squad:String(values.faction_squad??entry.faction_squad??'').trim().slice(0,100),origin:String(values.origin??entry.origin??'').trim().slice(0,200),backstory:String(values.backstory??entry.backstory??'').trim().slice(0,3500),personality:String(values.personality??entry.personality??'').trim().slice(0,1000),goals:String(values.goals??entry.goals??'').trim().slice(0,1000),extra_lore:String(values.extra_lore??entry.extra_lore??'').trim().slice(0,1000),rp_status:['Open to RP','Casual RP','Not Seeking RP'].includes(values.rp_status)?values.rp_status:entry.rp_status,category:cat.label,image_caption:String(values.image_caption??entry.image_caption??'').trim().slice(0,1500),updated_at:new Date().toISOString()};if(!patch.character_name||!patch.backstory)throw new Error('Character name and backstory are required.');const{data,error}=await dbRef.from(LORE_TABLE).update(patch).eq('id',entry.id).select('*').single();if(error)throw error;await updateLoreMessage(data);await updateDirectory(ctx.guildId);return data;}
async function portalToggleLore(ctx){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const{data,error}=await dbRef.from(LORE_TABLE).update({is_published:!entry.is_published,updated_at:new Date().toISOString()}).eq('id',entry.id).select('*').single();if(error)throw error;await updateLoreMessage(data);await updateDirectory(ctx.guildId);return data;}
async function portalDeleteLore(ctx){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const channel=clientRef?.guilds?.cache?.get(String(entry.guild_id))?.channels?.cache?.get(String(entry.channel_id));const msg=await channel?.messages?.fetch(String(entry.message_id)).catch(()=>null);await msg?.delete().catch(()=>{});const{error}=await dbRef.from(LORE_TABLE).delete().eq('id',entry.id);if(error)throw error;await updateDirectory(ctx.guildId);return{ok:true};}
async function portalSetLoreImages(ctx,values){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const files=decodePortalLoreImages(values.images,`lore-${entry.id}`);const channel=clientRef?.guilds?.cache?.get(String(entry.guild_id))?.channels?.cache?.get(String(entry.channel_id));const msg=await channel?.messages?.fetch(String(entry.message_id)).catch(()=>null);if(!msg)throw new Error('The lore message could not be found.');let urls=[];const caption=String(values.image_caption??entry.image_caption??'').trim().slice(0,1500);if(files.length){const uploaded=await msg.edit({embeds:loreEmbeds({...entry,image_urls:[],image_caption:caption}),files,attachments:[]});urls=[...uploaded.attachments.values()].map(a=>a.url).slice(0,MAX_IMAGES);}else await msg.edit({embeds:loreEmbeds({...entry,image_urls:[],image_caption:caption}),attachments:[]});const{data,error}=await dbRef.from(LORE_TABLE).update({image_urls:urls,image_caption:caption,updated_at:new Date().toISOString()}).eq('id',entry.id).select('*').single();if(error)throw error;await updateLoreMessage(data);await updateDirectory(ctx.guildId);return data;}
async function portalAdminLore(ctx,values){if(!ctx.isAdmin)throw new Error('Admin access required.');const entry=await getLoreById(ctx.guildId,values.id);if(!entry)throw new Error('Lore profile not found.');if(values.action==='delete'){const channel=clientRef?.guilds?.cache?.get(String(entry.guild_id))?.channels?.cache?.get(String(entry.channel_id));const msg=await channel?.messages?.fetch(String(entry.message_id)).catch(()=>null);await msg?.delete().catch(()=>{});const{error}=await dbRef.from(LORE_TABLE).delete().eq('id',entry.id);if(error)throw error;await updateDirectory(ctx.guildId);return{ok:true};}if(values.action==='toggle'){const{data,error}=await dbRef.from(LORE_TABLE).update({is_published:!entry.is_published,updated_at:new Date().toISOString()}).eq('id',entry.id).select('*').single();if(error)throw error;await updateLoreMessage(data);await updateDirectory(ctx.guildId);return data;}throw new Error('Unknown lore moderation action.');}
