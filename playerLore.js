...
async function getPortalLoreMediaChannel(guildId) {
  const guild = clientRef?.guilds?.cache?.get(String(guildId));
  const channel = guild?.channels?.cache?.get(String(PORTAL_MEDIA_CHANNEL_ID)) || guild?.channels?.cache?.get(String(COMMAND_CENTER_CHANNEL_ID));
  if (!channel?.isTextBased()) throw new Error('Watcher media storage is unavailable.');
  return channel;
}
async function getLoreStoredMessage(entry) {
  if (!entry?.channel_id || !entry?.message_id || String(entry.message_id) === 'portal') return null;
  const guild = clientRef?.guilds?.cache?.get(String(entry.guild_id));
  const channel = guild?.channels?.cache?.get(String(entry.channel_id));
  return channel?.messages ? channel.messages.fetch(String(entry.message_id)).catch(() => null) : null;
}
...
function decodePortalLoreImages(images,prefix){if(!Array.isArray(images))return[];return images.slice(0,MAX_IMAGES).map((item,index)=>{const raw=String(item?.data||'');const m=raw.match(/^data:(image\/[a-zA-Z0-9.+-]+);base64,(.+)$/s);if(!m)throw new Error(`Image ${index+1} is invalid.`);const buf=Buffer.from(m[2],'base64');if(!buf.length||buf.length>8*1024*1024)throw new Error(`Image ${index+1} must be smaller than 8 MB.`);const ext=String(item?.name||'').match(/\.[a-z0-9]{2,5}$/i)?.[0]||(m[1].includes('jpeg')?'.jpg':'.png');return{attachment:buf,name:`${prefix}-${index+1}${ext}`};});}
async function portalCreateLore(ctx,values){const existing=await getLoreByOwner(ctx.guildId,ctx.discordId);if(existing)throw new Error('You already have a lore profile.');const name=String(values.character_name||'').trim().slice(0,80);const backstory=String(values.backstory||'').trim().slice(0,3500);if(!name||!backstory)throw new Error('Character name and backstory are required.');const now=new Date().toISOString();const cat=LORE_CATEGORIES.find(c=>c.key===String(values.category)||c.label===String(values.category))||LORE_CATEGORIES[0];const row={guild_id:String(ctx.guildId),owner_id:String(ctx.discordId),owner_display:String(ctx.displayName||'Player').slice(0,100),character_name:name,title_nickname:String(values.title_nickname||'').trim().slice(0,100),faction_squad:String(values.faction_squad||'').trim().slice(0,100),origin:String(values.origin||'').trim().slice(0,200),backstory,personality:String(values.personality||'').trim().slice(0,1000),goals:String(values.goals||'').trim().slice(0,1000),extra_lore:String(values.extra_lore||'').trim().slice(0,1000),rp_status:['Open to RP','Casual RP','Not Seeking RP'].includes(values.rp_status)?values.rp_status:'Casual RP',category:cat.label,image_urls:[],image_caption:String(values.image_caption||'').trim().slice(0,1500),is_published:true,channel_id:String(PORTAL_MEDIA_CHANNEL_ID),message_id:'portal',created_at:now,updated_at:now};const{data,error}=await dbRef.from(LORE_TABLE).insert(row).select('*').single();if(error)throw error;return data;}
async function portalUpdateLore(ctx,values){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const cat=LORE_CATEGORIES.find(c=>c.label===values.category||c.key===values.category)||categoryForEntry(entry);const patch={character_name:String(values.character_name??entry.character_name).trim().slice(0,80),title_nickname:String(values.title_nickname??entry.title_nickname??'').trim().slice(0,100),faction_squad:String(values.faction_squad??entry.faction_squad??'').trim().slice(0,100),origin:String(values.origin??entry.origin??'').trim().slice(0,200),backstory:String(values.backstory??entry.backstory??'').trim().slice(0,3500),personality:String(values.personality??entry.personality??'').trim().slice(0,1000),goals:String(values.goals??entry.goals??'').trim().slice(0,1000),extra_lore:String(values.extra_lore??entry.extra_lore??'').trim().slice(0,1000),rp_status:['Open to RP','Casual RP','Not Seeking RP'].includes(values.rp_status)?values.rp_status:entry.rp_status,category:cat.label,image_caption:String(values.image_caption??entry.image_caption??'').trim().slice(0,1500),updated_at:new Date().toISOString()};if(!patch.character_name||!patch.backstory)throw new Error('Character name and backstory are required.');const{data,error}=await dbRef.from(LORE_TABLE).update(patch).eq('id',entry.id).select('*').single();if(error)throw error;const msg=await getLoreStoredMessage(data);if(msg)await msg.edit({content:`Portal media storage • Player Lore • ${data.character_name}`,allowedMentions:{parse:[]}}).catch(()=>{});return data;}
async function portalToggleLore(ctx){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const{data,error}=await dbRef.from(LORE_TABLE).update({is_published:!entry.is_published,updated_at:new Date().toISOString()}).eq('id',entry.id).select('*').single();if(error)throw error;return data;}
async function portalDeleteLore(ctx){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const msg=await getLoreStoredMessage(entry);await msg?.delete().catch(()=>{});const{error}=await dbRef.from(LORE_TABLE).delete().eq('id',entry.id);if(error)throw error;return{ok:true};}
async function portalSetLoreImages(ctx,values){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const preserveExisting=values?.preserveExisting===true;const files=decodePortalLoreImages(values.images,`lore-${entry.id}`);let msg=await getLoreStoredMessage(entry);if(!msg&&files.length){const channel=await getPortalLoreMediaChannel(ctx.guildId);msg=await channel.send({content:`Portal media storage • Player Lore • ${entry.character_name}`,allowedMentions:{parse:[]}});}let urls=Array.isArray(entry.image_urls)?entry.image_urls.filter(Boolean).slice(0,MAX_IMAGES):[];const caption=String(values.image_caption??entry.image_caption??'').trim().slice(0,1500);if(msg&&files.length){const uploaded=await msg.edit({content:`Portal media storage • Player Lore • ${entry.character_name}`,embeds:[],files,attachments:[]});urls=[...uploaded.attachments.values()].map(a=>a.url).slice(0,MAX_IMAGES);}else if(msg&&!files.length&&!preserveExisting){await msg.delete().catch(()=>{});msg=null;urls=[];}const{data,error}=await dbRef.from(LORE_TABLE).update({image_urls:urls,image_caption:caption,channel_id:msg?String(msg.channelId):String(PORTAL_MEDIA_CHANNEL_ID),message_id:msg?String(msg.id):'portal',updated_at:new Date().toISOString()}).eq('id',entry.id).select('*').single();if(error)throw error;return data;}
async function portalAdminLore(ctx,values){if(!ctx.isAdmin)throw new Error('Admin access required.');const entry=await getLoreById(ctx.guildId,values.id);if(!entry)throw new Error('Lore profile not found.');if(values.action==='delete'){const msg=await getLoreStoredMessage(entry);await msg?.delete().catch(()=>{});const{error}=await dbRef.from(LORE_TABLE).delete().eq('id',entry.id);if(error)throw error;return{ok:true};}if(values.action==='toggle'){const{data,error}=await dbRef.from(LORE_TABLE).update({is_published:!entry.is_published,updated_at:new Date().toISOString()}).eq('id',entry.id).select('*').single();if(error)throw error;return data;}throw new Error('Unknown lore moderation action.');}

module.exports={startPlayerLore,handlePlayerLoreCommand,handlePlayerLoreMessage,handlePlayerLoreInteraction,handlePlayerLoreModal,portalCreateLore,portalUpdateLore,portalToggleLore,portalDeleteLore,portalSetLoreImages,portalAdminLore};
