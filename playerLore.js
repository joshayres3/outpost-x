...
async function portalSetLoreImages(ctx,values){const entry=await getLoreByOwner(ctx.guildId,ctx.discordId);if(!entry)throw new Error('You do not have a lore profile yet.');const files=decodePortalLoreImages(values.images,`lore-${entry.id}`);let msg=await getLoreStoredMessage(entry);if(!msg&&files.length){const channel=await getPortalLoreMediaChannel(ctx.guildId);msg=await channel.send({content:`Portal media storage • Player Lore • ${entry.character_name}`,allowedMentions:{parse:[]}});}let urls=[];const caption=String(values.image_caption??entry.image_caption??'').trim().slice(0,1500);if(msg&&files.length){const uploaded=await msg.edit({content:`Portal media storage • Player Lore • ${entry.character_name}`,embeds:[],files,attachments:[]});urls=[...uploaded.attachments.values()].map(a=>a.url).slice(0,MAX_IMAGES);}else if(msg&&!files.length){await msg.delete().catch(()=>{});msg=null;}const{data,error}=await dbRef.from(LORE_TABLE).update({image_urls:urls,image_caption:caption,channel_id:msg?String(msg.channelId):String(PORTAL_MEDIA_CHANNEL_ID),message_id:msg?String(msg.id):'portal',updated_at:new Date().toISOString()}).eq('id',entry.id).select('*').single();if(error)throw error;return data;}
